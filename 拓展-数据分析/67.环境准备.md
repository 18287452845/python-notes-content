...
![](assets/download-anaconda-20250605165225-5dvenaj.png)
...
![](assets/anaconda_navigator-20250605165226-q6tf8je.png)
...
![](assets/JupyterLab_1-20250605165226-gl23sbr.png)

接下来，我们就可以编写代码、撰写文档和运行程序啦，如下图所示。

![](assets/JupyterLab_2-20250605165226-ug5bz9e.png)
...
2. 获得帮助。如果希望了解一个对象（如变量、类、函数等）的相关信息或使用方式，可以在对象后面使用`?`并运行代码， 窗口下方会显示出对应的信息，帮助我们了解该对象，如下所示。
   ...
   ![](assets/JupyterLab_3-20250605165226-df36txh.png)
3. 搜索命名。如果只记得一个类或一个函数名字的一部分，可以使用通配符`*`并配合`?`进行搜索，如下所示。

   ![](assets/JupyterLab_4-20250605165226-06glisc.png)
...
5. 魔法指令。JupyterLab 中有很多非常有趣且有用的魔法指令，例如可以使用`%timeit`测试语句的执行时间，可以使用`%pwd`查看当前工作目录等。如果想查看所有的魔法指令，可以使用`%lsmagic`，如果了解魔法指令的用法，可以使用`%magic`来查看，如下图所示。
   ...
   ![](assets/JupyterLab_5-20250605165226-6k6iumy.png)
