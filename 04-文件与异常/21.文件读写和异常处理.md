...
![](assets/20210803201644-20250605165224-o9qe33x.png)
...
上面代码的输入是磁盘上的`致橡树.txt`文件，`file.read()`读取文件中的全部文本，`print`把读取到的内容输出到屏幕；最后调用`close`关闭文件。文件操作完成后及时关闭文件，可以释放系统资源，也可以避免后续写入内容没有正确保存。
