...
![](assets/10_dataset_visualization-20250605165227-7p4f0cn.png)
...
2. **特征转换**：将原始数据转化为更适合模型输入的形式。常见的特征转换方法有：
   ...
   - 标准化和归一化：标准化将数据转换为均值为`0`、标准差为`1`的分布；归一化将数据缩放到特定的范围（如：$\small{[0, 1]}$），确保各特征在同一数量级，对应的公式如下所示：
     ...
     $$
     ^{\prime} = \frac{X - \mu}{\sigma}
     $$

     $$
     ^{\prime} = \frac{X - X_{min}}{X_{max} - X_{min}}
     $$
...
![](assets/10_test_web_api-20250605165227-lhfaxx0.png)
